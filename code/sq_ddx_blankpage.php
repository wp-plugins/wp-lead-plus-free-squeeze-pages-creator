...
<head>
    ...
    <?php echo vgt_de_serialize_data(get_option("vgt_custom_tracking_code")); ?>
    <?php vgt_write_custom_js($id, "before_head_close"); ?>
...
<script src="<?php echo get_option('vgt_wpl_plugin_url') . '/js/video.js' ?>"></script>
<!-- include tracking js -->
...
<?php if ($background_type == "video_bg"){ echo "<script> jQuery(document).ready(function(){ jQuery('body').tubular({videoId : '".vgt_get_YT_ID($background_value)."' }); });  </script>"; }
else if ($background_type == "image") { echo '<script>jQuery(document).ready(function(){ $("body").backstretch("'.$background_value.'"); });</script>'; } ?>
